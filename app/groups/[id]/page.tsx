...
export default async function GroupPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col flex-1 pb-8">
      ...
      <div className="px-5 mt-6">
        ...
        <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full mb-3 ${
          group.status === "confirmed"
            ...
            : "bg-status-forming/10 text-status-forming border border-status-forming/20"
        }`}>
          <span className={`w-1.5 h-1.5 rounded-full ${group.status === "confirmed" ? "bg-status-verified" : "bg-status-forming"}`} />
        ...
        {event && (
          <p className="text-sm text-ink/50 mt-0.5">{event.festival_name} · {event.name}</p>
        )}

        {/* Progress toward chat unlock */}
        {!chatUnlocked && (
          <div className="mt-4 bg-sunken rounded-xl p-4">
            <div className="flex justify-between items-center mb-2">
              <p className="text-xs text-ink/50">Progress to group chat</p>
              <p className="text-xs text-ink font-medium">{confirmed.length} / 2 confirmed</p>
            </div>
        ...
        {chatUnlocked && (
          <div className="mt-4 inline-flex items-center gap-1.5 text-xs font-medium text-brand-500">
          ...
          {confirmed.map(({ users: u }) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ink/40 truncate">{getSocialHandle(u)}</p>
              </div>
      ...
      {isPendingMember && (
        <div className="px-5 mt-6">
          <div className="bg-card rounded-2xl p-5 text-center border border-sunken">
